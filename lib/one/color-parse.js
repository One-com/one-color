...
one.color.fromCSSRGBA = function (strCSS) {
    strCSS = strCSS.replace(/^rgba?\(|\)|\s$/g, '');
    var valid = true,
        channels = strCSS.split(','),
        i;

    for (i = 0; i < channels.length; i++) {
        if (/%/.test(channels[i])) { // Float percentage
            channels[i] = parseFloat(channels[i]) / 100;
        } else if (i === 3) { // Alpha channel
            channels[i] = parseFloat(channels[i]);
        } else { // Integer 0-255
            channels[i] = parseInt(channels[i], 10) / 255;
        }

        if (channels[i] && (isNaN(channels[i]) || channels[i] < 0 || channels[i] > 1)) {
            valid = false;
            break;
        }
    }
    ...
    if (valid) {
        return new one.color.RGB(channels[0], channels[1], channels[2], channels[3]);
    } else {
        return false;
    }
};
...
one.color.parse = function (obj) {
    if (obj.charCodeAt) {
        if (/rgb/i.test(obj)) {
            return one.color.fromCSSRGBA(obj);
        } else {
